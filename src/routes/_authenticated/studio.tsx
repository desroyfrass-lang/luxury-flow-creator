...
import {
  ...
  describeDepthChange,
  type ControlDepthId,
} from "@/lib/studio/control-depths";
  ...
  head: () => ({
    meta: [
      { title: "Frass Vision Studios (FV Studios) — Frass Hill" },
      {
        name: "description",
        content:
          "Enter Frass Vision Studios to plan, direct, edit and finish one production with Frassy beside you.",
      },
      { property: "og:title", content: "Frass Vision Studios (FV Studios) — Frass Hill" },
      {
        property: "og:description",
        content: "A creator-first production room for Frass Originals and every Builder story.",
      },
...
const CREATION_DOORS = [
  {
    ...
    destination: "podcast",
    lane: "audio",
    note: "Episodes, interviews, spoken work",
  },
  {
    id: "music",
    label: "Music",
    ...
    lane: "audio",
    note: "Songs, scores, sound-led work",
  },
  {
    id: "music-video",
    label: "Video / Music Video",
    icon: MonitorPlay,
    destination: "youtube",
    lane: "visual",
    note: "Visual stories and performances",
  },
  {
    id: "animation",
    label: "Animation",
    ...
    destination: "youtube",
    lane: "visual",
    note: "Characters, scenes, movement",
  },
  {
    id: "film",
    ...
    destination: "internal",
    lane: "visual",
    note: "Long-form and real-world film",
  },
  {
    id: "photography",
    ...
    destination: "internal",
    lane: "visual",
    note: "Shoots, campaigns, image sets",
  },
] as const;
...
function StudioPage() {
  ...
  const add = useMutation({
    mutationFn: () =>
      createProject({
        data: { title: newTitle.trim() || `Untitled ${task?.label ?? "production"}`, destination },
      }),
    onSuccess: (p) => {
      setNewTitle("");
      setActiveId(p.id);
      ...
      setSurfaced({
        kind: "done",
        title: "Production open",
        body: "Nothing was charged — opening a production is always free. Your next step is waiting in the command centre.",
      });
    },
    onError: (e: Error) =>
      setSurfaced({ kind: "blocked", title: "Could not open the production", body: e.message }),
  });
  ...
  const run = useMutation({
    ...
    onError: (e: Error) =>
      setSurfaced({ kind: "blocked", title: "That could not run", body: e.message }),
  });
  ...
  const depth = useMutation({
    ...
    onSuccess: (p) => {
      toast.success(
        describeDepthChange(
          controlDepth(active?.control_depth).id,
          controlDepth(p.control_depth).id,
        ).message,
      );
      void qc.invalidateQueries({ queryKey: ["studio-projects"] });
    },
    onError: (e: Error) =>
      setSurfaced({ kind: "blocked", title: "Control depth unchanged", body: e.message }),
  });

  const runPhone = useMutation({
    mutationFn: async ({ report, file }: { report: QualityReport; file: File }) => {
      if (!active) throw new Error("Open a production first.");
      const forecast = buildForecast("Enhance Phone Recording", [
        { key: "voice-enhance", qty: report.minutes },
      ]);
      ...
      const sourceUpload = await supabase.storage
        .from(A1_CLEAN_BUCKET)
        .upload(prepared.source, file, { contentType: file.type, upsert: false });
      if (sourceUpload.error)
        throw new Error(`Source preservation failed: ${sourceUpload.error.message}`);
      const outputUpload = await supabase.storage
        .from(A1_CLEAN_BUCKET)
        .upload(prepared.output, output, { contentType: "audio/wav", upsert: false });
      ...
      const signed = await supabase.storage
        .from(A1_CLEAN_BUCKET)
        .createSignedUrl(prepared.output, 3600);
      return { verified, url: signed.data?.signedUrl ?? null };
    },
    onMutate: () =>
      setSurfaced({
        kind: "working",
        title: "Enhance Phone Recording is running",
        body: "Your recording is being cleaned on this device. The source file is kept untouched.",
      }),
    ...
    onError: (e: Error) =>
      setSurfaced({
        kind: "blocked",
        title: "Enhance Phone Recording did not finish",
        body: `${e.message} Nothing was charged.`,
      }),
  });
  ...
  const primary = needsProduction
    ? {
        label: task ? "Open this production" : "Choose what you're making",
        onClick: () => {
          setTab("create");
          if (task) add.mutate();
        },
  ...
  return (
    <SiteShell>
      <div
        className={`fv-studio min-h-screen overflow-x-clip pb-24 text-foreground lg:pb-0 ${arrival ? "" : "fv-lights-up"}`}
      >
        {arrival ? (
          <button
            type="button"
          ...
          >
            <img
              src={studioEntry}
              alt="Eye-level entrance into the FV Studios production room"
            ...
            <span className="absolute inset-0 grid place-items-center px-6 text-center">
              <span className="block">
                <span className="block font-display text-4xl uppercase leading-none sm:text-6xl">
        ...
        <header className="fv-studio-header sticky top-0 z-30 backdrop-blur-xl">
          <div className="mx-auto grid max-w-[1600px] grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 pb-2 pt-11 sm:px-5">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="hidden shrink-0 rounded-full border border-border px-2.5 py-1 text-xs uppercase text-muted-foreground sm:inline">
                {controlDepth(active?.control_depth).label}
              </span>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => setCredits(true)} className="h-10">
                {(w?.balance ?? 0).toLocaleString()} cr
              </Button>
              {isAdmin === true ? (
                <Button size="sm" asChild className="fv-secondary-control h-10 rounded-full">
                  <Link to="/studios" aria-label="Founder Originals">
                    <Shield className="h-4 w-4" />{" "}
                    <span className="hidden sm:inline">Founder Originals</span>
                  </Link>
                </Button>
              ) : null}
            </div>
          ...
          <nav
            ...
            className="fv-studio-nav mx-auto flex max-w-[1600px] gap-1 overflow-x-auto px-3 pb-2 sm:px-5"
          >
            {TABS.map((t) => {
              const Icon = t.icon;
              const on = t.id === tab;
            ...
            <button
              ...
              className="fv-studio-tab ml-auto flex min-h-11 shrink-0 items-center gap-2 px-3 text-sm text-muted-foreground"
            >
        ...
        >
          ...
          <section className="relative z-10 min-w-0 space-y-3" aria-label="Command centre">
            <div className="fv-control-room relative overflow-hidden">
              ...
              <div className="fv-studio-speaker fv-speaker-right" aria-hidden="true">
                <span />
                <span />
              </div>
            ...
            <div className="fv-action-deck grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 py-3 sm:flex">
              <Button
                ...
                className="fv-primary-action min-h-14 min-w-0 text-base sm:min-w-72"
              >
              ...
              {active ? (
                <Button
                  variant="ghost"
                  className="fv-quiet-action min-h-12 shrink-0"
                  onClick={() => {
                    setTask(null);
                    setCreating(true);
                    setTab("create");
                ...
                >
                  <Plus /> <span className="hidden sm:inline">New production</span>
                </Button>
              ) : null}
              <span className="col-span-2 text-xs text-muted-foreground sm:ml-auto">
                Nothing runs or charges before approval.
              </span>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {active
                      ...
                      : "Open a production in Create first."}
                  </p>
                </div>
              ) : null}
              {tab === "assets" ? (
          ...
          >
            <button
              ...
              className="fv-frassy-portrait group relative mx-auto block w-full max-w-[19rem] overflow-hidden text-left"
              aria-label="Talk to Frassy in the studio"
            >
              <span
                className="fv-frassy-halo absolute inset-x-[8%] bottom-[4%] h-[62%]"
                aria-hidden="true"
              />
              <img
            ...
            <div className="fv-director-console mt-[-1rem] p-4 pt-7">
              <form
                ...
                onSubmit={(e) => {
                  ...
                  setWhy(false);
                  setSurfaced({ kind: "forecast", plan: planFromDirection(direction) });
                }}
              >
                <label className="text-sm text-muted-foreground" htmlFor="director-direction">
                ...
                <textarea
                  ...
                  placeholder="Tell Frassy what you want to make…"
                  className="fv-director-input mt-2 w-full resize-none rounded-lg p-3 text-sm outline-none"
                />
                <Button
                  type="submit"
                  disabled={!direction.trim()}
                  className="fv-secondary-control mt-2 min-h-12 w-full"
                >
                  <Send /> Show my plan and cost
                </Button>
              </form>
              {directed ? null : (
        ...
        <button
          type="button"
          onClick={summonFrassy}
          className="fv-frassy-mobile fixed bottom-[5.35rem] right-3 z-40 h-16 w-16 overflow-hidden rounded-full lg:hidden"
          aria-label="Talk to Frassy in the studio"
        >
          <img
            src={FV_STUDIOS_FRASSY_LOOK.image}
        ...
        <div className="fv-studio-surface fixed inset-x-0 bottom-0 z-30 bg-card/95 p-3 backdrop-blur lg:hidden">
          <Button onClick={primary.onClick} className="fv-primary-action min-h-12 w-full">
            {primary.label} <ArrowRight />
          </Button>
        </div>
      </div>
      ...
      <Dialog open={credits} onOpenChange={setCredits}>
        <DialogContent className="fv-studio-surface">
          <DialogHeader>
            <DialogTitle>Credits & receipts</DialogTitle>
            <DialogDescription>
              {(w?.balance ?? 0).toLocaleString()} credits available.
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-3 gap-2 text-center text-sm">
            <div>
              <p className="text-xs text-muted-foreground">Today</p>
              <p>{(w?.today_used ?? 0).toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Month</p>
              <p>{(w?.month_used ?? 0).toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Projected</p>
              <p>{projected.toLocaleString()}</p>
            </div>
          </div>
          <div className="space-y-2">
            ...
            {(ledgerQ.data ?? []).length === 0 ? (
              <p className="text-sm text-muted-foreground">No AI work billed yet.</p>
            ) : null}
          </div>
          <details>
            <summary className="min-h-11 cursor-pointer text-sm font-medium">
              Always free in the Studio
            </summary>
          ...
          <Button variant="outline" asChild className="min-h-12 w-full">
            <Link to="/financial-center">Open Creator Wallet</Link>
          </Button>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={further} onOpenChange={setFurther}>
        <DialogContent className="fv-studio-surface">
          <DialogHeader>
            <DialogTitle>Take it further</DialogTitle>
            <DialogDescription>
              Business and earning possibilities sit beside finished work, never in front of
              creation.
            </DialogDescription>
          </DialogHeader>
          {active ? (
            ...
            <p className="text-sm text-muted-foreground">
              Open a production to see relevant possibilities.
            </p>
          )}
          <Button variant="outline" asChild className="min-h-12 w-full">
            <Link to="/business-builder">
              Open Business Builder <ChevronRight />
            </Link>
          </Button>
          {isAdmin === true ? (
            <div className="grid gap-2 border-t border-border pt-3 sm:grid-cols-2">
              <Button size="sm" variant="outline" asChild>
                <Link to="/studios/productions">
                  <Film /> Productions
                </Link>
              </Button>
              <Button size="sm" variant="outline" asChild>
                <Link to="/studios/series">
                  <BookOpen /> Series Bibles
                </Link>
              </Button>
              <Button size="sm" variant="outline" asChild>
                <Link to="/studios/characters">
                  <Layers /> Characters
                </Link>
              </Button>
              <Button size="sm" variant="outline" asChild>
                <Link to="/studios/assets">
                  <ImageIcon /> Assets & locations
                </Link>
              </Button>
            </div>
          ) : null}
...
function CreateWorkspace({
          ...
          {CREATION_DOORS.map((door) => {
            ...
            return (
              <Button
                ...
                className="fv-studio-door h-auto min-h-24 justify-start whitespace-normal p-4 text-left"
              >
                <span className="fv-door-icon grid h-11 w-11 shrink-0 place-items-center rounded-full">
                  <Icon className="h-5 w-5 text-accent" />
                </span>
                <span className="min-w-0">
                  <span className="block text-sm">{door.label}</span>
    ...
    return (
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_200px_auto]">
        <label className="grid gap-1 text-sm text-muted-foreground">
          Name this {task?.label}
          <input
            autoFocus
            value={newTitle}
            onChange={(e) => onTitle(e.target.value)}
            placeholder={`Untitled ${task?.label}`}
            className="h-12 rounded-md border border-input bg-background px-3 text-foreground outline-none focus:border-accent"
          />
        </label>
        <label className="grid gap-1 text-sm text-muted-foreground">
          Destination
        ...
        <Button
          disabled={adding}
          onClick={onOpen}
          className="mt-auto min-h-12 bg-accent text-accent-foreground hover:bg-accent/90"
        >
          <Plus /> {adding ? "Opening…" : "Open production"}
        </Button>
      </div>
    );
  }
          ...
          {CREATION_DOORS.map((door) => {
            ...
            return (
              <Button
                ...
                variant="ghost"
                onClick={() => onChoose(door)}
                className="fv-studio-door h-auto min-h-20 justify-start whitespace-normal p-3 text-left"
              >
                <span className="fv-door-icon grid h-10 w-10 shrink-0 place-items-center rounded-full">
                  <Icon className="h-5 w-5 text-accent" />
                </span>{" "}
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-sm text-muted-foreground">
        The Frass machine for this kind of work is NOT INSTALLED yet, so nothing here can generate
        media or charge you. Planning, writing and manual editing are open now.
      </p>
      <Button variant="outline" onClick={onAskFrassy} className="min-h-12">
        Plan it with Frassy
      </Button>
    </div>
  );
    ...
    >
      <DialogContent className="fv-studio-surface max-h-[85vh] overflow-auto">
        {surfaced?.kind === "forecast" ? (
          <>
            <DialogHeader>
              <DialogTitle>Here is the plan and the cost</DialogTitle>
              <DialogDescription>{surfaced.plan.understanding}</DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              {surfaced.plan.forecast.lines.map((l) => (
                <div key={l.key} className="flex items-start justify-between gap-3 text-sm">
                  <span className="text-muted-foreground">
                    {l.label} · {unitLabel(l.unit, l.qty)}
                  </span>
                  <span>{l.credits.toLocaleString()}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-border pt-3">
              <p className="text-xl text-accent">
                {surfaced.plan.forecast.total.toLocaleString()} Credits
              </p>
              <p className="text-xs text-muted-foreground">
                ≈ {usdFor(surfaced.plan.forecast.total)} of compute · about{" "}
                {formatDuration(surfaced.plan.forecast.seconds)} to process · balance after{" "}
                {(balance - surfaced.plan.forecast.total).toLocaleString()}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                onClick={() => onApprove(surfaced.plan)}
                disabled={running || balance < surfaced.plan.forecast.total}
                className="min-h-12 bg-accent text-accent-foreground hover:bg-accent/90"
              >
                {running
                  ? "Sending…"
                  : balance >= surfaced.plan.forecast.total
                    ? "Approve"
                    : "Not enough credits"}
              </Button>
            ...
            {why ? (
              <div className="border border-border bg-background/60 p-3 text-sm text-muted-foreground">
                {surfaced.plan.reasoning.map((r) => (
                  <p key={r} className="mb-2">
                    {r}
                  </p>
                ))}
                <p>{surfaced.plan.manual}</p>
              </div>
            ) : null}
          </>
        ) : null}
        ...
        {surfaced && surfaced.kind !== "forecast" ? (
          <>
            <DialogHeader>
              <DialogTitle>{surfaced.title}</DialogTitle>
              <DialogDescription>{surfaced.body}</DialogDescription>
            </DialogHeader>
            <div className="flex flex-wrap gap-2">
