...
export const Route = createFileRoute("/frass-kicks/side-kicks/men")({
  head: () => ({
    meta: [
      { title: "Side Kicks — Men" },
      { name: "description", content: "Men's Side Kicks collections." },
      ...
      <section className="mx-auto max-w-[1600px] px-6 lg:px-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {CARDS.map(([slug, title], i) => (
            <CollectionCard key={slug} to="/collection/$handle" params={{ handle: `side-kicks-men-${slug}` }} image={IMAGES[i % IMAGES.length]} eyebrow="Men" title={title} size="md" />
          ))}
        </div>
      </section>
    </SiteShell>
