...
| FRASS-0473 | Pre-launch certification: roles, RLS isolation, financial integrity, email queue, file uploads | 1 Critical (member-set platform allocation on receipts); 2 High (visual-search accepted any member's storage path and any file type; try-on server fetched unbounded remote URLs); 3 Medium (no shared MIME/size guard across pickers; storage buckets carry no bucket-level limits; feedback/admin uploads unvalidated); Low/Info: extension in public schema, `expected_platform_allocation` executable by signed-in users by design | Critical + High fixed. Receipt policies now pin platform allocation to the official rate; uploads validated by `src/lib/uploads.ts` (client) and `src/lib/uploads.server.ts` (server ownership + MIME + size re-check); try-on remote fetch restricted to https images ≤12MB. Residual Medium: bucket-level limits cannot be set from Lovable Cloud tooling — enforced in application code instead. |


---

## 9. FRASS-0505 — Security Review Cycle (every major release)

Triggered automatically by any major release, whether or not anything is broken:
